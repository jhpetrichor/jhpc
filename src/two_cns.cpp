  ...
  private:
    ...
    set<set<string>> _m_find_complex() {
        ...
        for(int i = 0; i < g->node_count; ++i) {
            ...
            if(g->density(temp_complex) < 0.6) continue;
            if(cohesiveness(temp_complex) < 0.3) continue;
            complexes.insert(complex);
        }
        return complexes;
    }

    double cohesiveness(const set<int>& complex) {
        double w_in = 0.0;
        double w_out = 0.0;
        for(auto i: complex) {
            set<int> neighbir = g->get_neighbor(i);
            for(auto j: neighbir) {
                if(complex.count(j)) {
                    w_in += attraction[i][j];
                } else {
                    w_out += attraction[i][j];
                }
            }
        }
