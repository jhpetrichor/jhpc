 ...
 * @LastEditTime: 2024-05-28 20:38:36
 */
...
// 使用go term为连边添加权重
void UnGraph::weight_by_go_term() {
    for (auto& edge : edges) {
        ...
        double weight = 0.5 * (child_ancestor.get_similarity_protein(go_term_1, go_term_2) + ancestor_child.get_similarity_protein(go_term_1, go_term_2));
        edge->weight = weight;
...
void Complex::update_complexes(vector<set<string>>& complexes,
    ...
    for (auto& c : complexes) {
        if (complex_match_score1(c, complex) >= threshold) {
            return;
        }
    }
