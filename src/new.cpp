...
int main() {
    ...
    std::vector<std::set<std::string>> complexes;
    for (auto &ppi: splitted_ppi) {
        UnGraph::get_complexes(ppi, complexes, 0.45);
        std::cout << complexes.size() << std::endl;
        std::cout << ppi.proteins.size() << "\t " << ppi.edges.size() << endl;
    }
