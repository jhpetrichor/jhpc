 ...
 * @LastEditTime: 2024-05-28 17:18:00
 */
...
void BioInformation::read_gene_expression() {
    ...
    if (!file.is_open()) {
        cout << "--- [ERROR]: Failed to open file: " << GENE_EXPRESSION
             << " ---" << endl;
        exit(1);
    }
...
void BioInformation::read_go_slim() {
    ...
    if (!file.is_open()) {
        cout << "--- [ERROR]: Failed to open file: " << GO_SLIM << " ---"
             << endl;
        exit(1);
    }
    string line;
...
void BioInformation::read_subcellular() {
    ...
    if (!file.is_open()) {
        cout << "--- [ERROR]: Failed to open file: " << SUBCELLULAR << " ---"
             << endl;
        exit(1);
    }
    string line;
...
double BioInformation::go_expression_pearson(const string& protein1,
                                             const string& protein2) {
    auto it1 = go_expression.find(protein1);
    auto it2 = go_expression.find(protein2);
    if (it1 == go_expression.end()) {
        // cout << "--- [ERROR]: Does not contain this protein in Gene
        // Expression!
        // --- " << protein1 << endl;
        return 0.0;
    }
...
double BioInformation::go_expression_spearman(const string& protein1,
                                              const string& protein2) {
    auto it1 = go_expression.find(protein1);
    auto it2 = go_expression.find(protein2);
    if (it1 == go_expression.end()) {
...
//
double BioInformation::go_slim_slim(const string& protein1,
                                    const string& protein2) {
    auto it1 = go_slim.find(protein1);
    auto it2 = go_slim.find(protein2);

    if (it1 == go_slim.end()) {
        // cout << "--- [WARNNING]: Does not contain this protein go_slim in GO
        // SLIM
        // ---" << protein1 << endl;
        return 0.0;
    }
    ...
    return static_cast<double>(pow(common_go_slim.size(), 2)) /
           static_cast<double>(it1->second.size() * it2->second.size());
}

double BioInformation::go_slim_slim_jaccard(const string& protein1,
                                            const string& protein2) {
    auto it1 = go_slim.find(protein1);
    auto it2 = go_slim.find(protein2);
    ...
    set_union(it1->second.begin(), it1->second.end(), it2->second.begin(),
              it2->second.end(), inserter(all_go_slim, all_go_slim.begin()));
    set_intersection(it1->second.begin(), it1->second.end(),
                     it2->second.begin(), it2->second.end(),
                     inserter(common_go_slim, common_go_slim.begin()));
    ...
    return static_cast<double>(common_go_slim.size()) / static_cast<double>(all_go_slim.size());
}