 ...
 * @LastEditTime: 2024-06-10 22:48:38
 */
...
void Protein::display() const {
    cout << id << "\t" << protein_name << "\tdegree: " << degree() << endl;
    cout << "neighbor\t";
    for (auto& nei : neighbor) {
        printf("%s(%d)\t", nei->protein_name.c_str(), nei->id);
    }
    std::cout << endl;
}
...
void Protein::add_neighbor(const ProteinPtr& _protein) {
    ...
    if (_protein->protein_name == this->protein_name)
        return;
...
double Protein::jaccard_similarity(const ProteinPtr& other) const {
    ...
    set_intersection(neighbor.begin(), neighbor.end(), other->neighbor.begin(),
                     other->neighbor.end(), inserter(common, common.begin()));
    return static_cast<double>(common.size()) /
           static_cast<double>(degree() + other->degree() - common.size());
}

double Protein::more_jaccard_similarity(const ProteinPtr& other) const {
    set<ProteinPtr> common;
    set_intersection(neighbor.begin(), neighbor.end(), other->neighbor.begin(),
                     other->neighbor.end(), inserter(common, common.begin()));
    return static_cast<double>(common.size() + 2) /
           static_cast<double>(degree() + other->degree() - common.size());
}

int Protein::degree() const { return static_cast<int>(neighbor.size()); }

pair<double, double> Protein::unidirectional_similarity(const ProteinPtr& other) const {
    set<ProteinPtr> common;
    set_intersection(neighbor.begin(), neighbor.end(),
                    other->neighbor.begin(), other->neighbor.end(),
                    inserter(common, common.begin()));
    const double s1 = static_cast<double>(common.size()) / static_cast<double>(degree());
    const double s2 = static_cast<double>(common.size()) / static_cast<double>(other->degree()); 
    return make_pair(s1, s2);
}

bool Protein::ProteinCompareByWeight(const ProteinPtr& p1,
                                     const ProteinPtr& p2) {
...
void Edge::display() const {
    ...
    cout << node_a->protein_name << "(" << node_a->id << ") -- "
         << node_b->protein_name << "(" << node_b->id << ")\t" << weight << "\t"
         << balanced_weight << std::endl;
}
...
UnGraph::UnGraph(string& ppi_file, Weight weight) {
    ...
    if (weight) {
        ...
        read_edge_list_with_weight(ppi_file, proteins_name, edge_list,
                                   edge_weight);
        assert(edge_list.size() % 2 == 0);
        ID2Protein.resize(proteins_name.size());
        connected.resize(proteins_name.size(),
                         vector<bool>(proteins_name.size(), false));
        // update node
        for (auto& protein_name : proteins_name) {
            ProteinPtr protein(
                new Protein(static_cast<int>(proteins.size()), protein_name));
            ID2Protein[proteins.size()] = protein; // ID2Protein
            Protein2ID[protein] =
                static_cast<int>(proteins.size()); // Protein2ID
            protein_name_id[protein_name] =
                static_cast<int>(proteins.size()); // protein_name_id
...
UnGraph::UnGraph(set<string>&& set_proteins, vector<string>&& list_edges,
                 vector<double>&& edge_weight) {
    assert(list_edges.size() % 2 == 0);
    ID2Protein.resize(set_proteins.size());
    connected.resize(set_proteins.size(),
                     vector<bool>(set_proteins.size(), false));
    // update node
...
int UnGraph::protein_count() const {
    return proteins.size();
}

int UnGraph::edge_count() const {
    return edges.size();
}
...
void UnGraph::display() const {
    ...
    cout << "edges: " << endl;
    for (auto& e : edges) {
        cout << e->node_a->protein_name << "\t" << e->node_b->protein_name
             << endl;
    }
...
void UnGraph::sort_edges_balanced_weight() {
    sort(edges.begin(), edges.end(), Edge::CompareByBalancedWeight);
    update_edge_id();
}

void UnGraph::update_edge_id() {
    for(int i = 0; i < edges.size(); ++i) {
        auto e = edges[i];
        set<string> e_set{e->node_a->protein_name, e->node_b->protein_name};
        Edge2ID[e_set]  = i;
    }
}

void UnGraph::read_edge_list(string& file_path, set<string>& proeins_list,
                             vector<string>& edge_list) {
...
void UnGraph::read_edge_list_with_weight(string& file_path,
                                         set<string>& proeins_list,
                                         vector<string>& edge_list,
                                         vector<double>& edge_weight) {
    fstream file(file_path);
    if (!file.is_open()) {
...
void UnGraph::add_edge(const ProteinPtr& protein_a, const ProteinPtr& protein_b,
                       double weight) {
...
EdgePtr UnGraph::getEdge(const ProteinPtr& protein_a,
                         const ProteinPtr& protein_b) {
...
// 使用go term为连边添加权重
// void Graph::weight_by_go_term() {
//     for (auto& edge : edges) {
//         string protein_1 = edge->node_a->protein_name;
//         string protein_2 = edge->node_b->protein_name;
//         // set<string> go_term_1 = protein_go[protein_1];
//         // set<string> go_term_2 = protein_go[protein_2];
//         double weight = 0.5 * (child_ancestor.get_similarity_protein(protein_1, protein_2) + ancestor_child.get_similarity_protein(protein_1, protein_2));
//         edge->weight = weight;
//         edge->balanced_weight = weight;
//     }
// }

void UnGraph::clean_protein_weight() {
    for (auto& p : proteins) {
        p->weight = 0.0;
    }
}
...
double UnGraph::agglomeration_coefficient(const vector<ProteinPtr>& nodes) {
    ...
    return static_cast<double>(edge_count * 2) /
...
void UnGraph::calculate_structure_similarty(
    vector<vector<double>>& ss_weight) const {
    ...
    for (auto it1 = proteins.begin(); it1 != proteins.end(); ++it1) {
        for (auto it2 = next(it1); it2 != proteins.end(); ++it2) {
            double ss_w =
                (JCS[(*it1)->id][(*it2)->id] + CNS[(*it1)->id][(*it2)->id]) /
                (common_neighbor_size[(*it1)->id][(*it2)->id] + 1);
    ...
    for (auto it1 = proteins.begin(); it1 != proteins.end(); ++it1) {
        for (auto it2 = next(it1); it2 != proteins.end(); ++it2) {
            ...
            set_intersection((*it1)->neighbor.begin(), (*it1)->neighbor.end(),
                             ...
                             inserter(common, common.begin()));
            common_size[(*it1)->id][(*it2)->id] =
                static_cast<int>(common.size());
            common_size[(*it2)->id][(*it1)->id] =
                static_cast<int>(common.size());
        }
    }
}

void UnGraph::get_JCS(vector<vector<double>>& JCS,
                      const vector<vector<int>>& common_size) const {
    JCS.resize(ID2Protein.size(), vector<double>(ID2Protein.size(), 0));
    for (auto it1 = proteins.begin(); it1 != proteins.end(); ++it1) {
        for (auto it2 = next(it1); it2 != proteins.end(); ++it2) {
            double jcs =
                static_cast<double>(common_size[(*it1)->id][(*it2)->id]) /
                static_cast<double>(((*it1)->neighbor.size() +
                                     (*it2)->neighbor.size() -
                                     common_size[(*it1)->id][(*it2)->id]));
...
void UnGraph::get_CNS(vector<vector<double>>& CNS,
                      const vector<vector<double>>& JCS) const {
    ...
    for (auto it1 = proteins.begin(); it1 != proteins.end(); ++it1) {
        for (auto it2 = next(it1); it2 != proteins.end(); ++it2) {
            ...
            for (const auto& neighbor : common) {
                cns += JCS[(*it1)->id][neighbor->id] +
                       JCS[(*it2)->id][neighbor->id];
            }
...
double UnGraph::homogeneity_index_jaccard(const ProteinPtr& a,
                                          const ProteinPtr& b) {
...
void UnGraph::calculate_attraction(vector<double>& attractions) const {
    ...
    for (auto& e : edges) {
        ...
        double attraction =
            weight_node[e->node_a->id] * weight_node[e->node_b->id] / pow(d, 2);
...
void UnGraph::calculate_walk_probability(
    vector<vector<double>>& probability) const {
    probability.resize(
        static_cast<int>(ID2Protein.size()),
        vector<double>(static_cast<int>(ID2Protein.size()), 0.0));
    ...
    for (auto& e : edges) {
        // a ---> b 的概率
        probability[e->node_a->id][e->node_b->id] =
            e->balanced_weight / node_weight[e->node_a->id];
        probability[e->node_b->id][e->node_a->id] =
            e->balanced_weight / node_weight[e->node_b->id];
    }
}
...
void UnGraph::split_graph(queue<UnGraph>& ppi_queue,
                          vector<UnGraph>& splited_ppi) {
    ...
    // 从小大到排列
    sort(current_ppi.edges.begin(), current_ppi.edges.end(),
         Edge::CompareByBalancedWeight);
    for (int i = current_ppi.edges.size() - 1; i >= 0; i--) {
        if (getfa1(current_ppi.edges[i]->node_a->id, fa) ==
            getfa1(current_ppi.edges[i]->node_b->id, fa)) {
            continue;
        }
        fa[getfa1(current_ppi.edges[i]->node_a->id, fa)] =
            getfa1(current_ppi.edges[i]->node_b->id, fa);
        count += 1;
    ...
    while (true) {
        ...
        for (int i = location + 1; i < current_ppi.ID2Protein.size(); ++i) {
            ...
            if (getfa1(current_ppi.ID2Protein[i]->id, fa) ==
                current_ppi.ID2Protein[i]->id) {
                location = i;
                success = true;
                break;
            }
        ...
        for (int i = 0; i < current_ppi.ID2Protein.size(); ++i) {
            if (getfa1(current_ppi.ID2Protein[i]->id, fa) ==
                current_ppi.ID2Protein[location]->id) {
...
void UnGraph::get_complexes(UnGraph& origin_ppi, UnGraph& g,
...
void UnGraph::get_complexes1(UnGraph& g, vector<set<string>>& complexes,
                             double similarity_threshold) {
    set<string> complex;
...
void UnGraph::write_to_file(const string& file_path) const {
    ...
    for (auto& e : edges) {
        file << e->node_a->protein_name << "\t" << e->node_b->protein_name
             << "\t" << e->balanced_weight << endl;
    }
    file.close();
    cout << "Write to file finished!" << endl;
}

void UnGraph::write_xgmml(string& file_path) {
    ofstream file(file_path);
    if (!file.is_open()) {
        cerr << "<ERROR>: Failed to open file! " << file_path << endl;
        exit(1);
    ...
    for(int i = 0; i < ID2Protein.size(); i++) {
        file << "  node [" << endl;
        file << "    id " << ID2Protein[i]->id << endl;
        file << "    label " << "\"" << ID2Protein[i]->protein_name << "\"" << endl;
        file << "  ]" << endl;
    }
    for(auto& e: edges) {
        file << "  edge [" << endl;
        file << "    source " << e->node_a->id << endl;
        file << "    target " << e->node_b->id << endl;
        file << "  ]" << endl;
    }
...
void UnGraph::normalize_edge_weight_min_max() {
    double max = 0.0;
    double min = MAXFLOAT;
    for(auto& e: edges) {
        if(e->balanced_weight > max) max = e->balanced_weight;
        if(e->balanced_weight < min) min = e->balanced_weight;
    }
    std::cout << "max: " << max << "\t min: " << min << endl;
    for(auto& e: edges) {
        // cout << e->balanced_weight << "\t";
        e->balanced_weight = (e->balanced_weight - min) / (max - min);
        // cout << e->balanced_weight << endl;
    }
}


bool UnGraph::compare_pairs(const pair<EdgePtr, int>& pair1,
                            const pair<EdgePtr, int>& pair2) {
...
double Complex::complex_match_score(Complex& other) {
    set<string> common;
    set_intersection(proteins.begin(), proteins.end(), other.proteins.begin(),
                     other.proteins.end(), inserter(common, common.begin()));
    double score =
        static_cast<double>(common.size()) /
        static_cast<double>(max(other.proteins.size(), proteins.size()));
    display();
    other.display();
    cout << score << endl;
    return score;
...
void Complex::write_complex_to_file(vector<Complex>& complexes,
                                    string& file_path) {
    ofstream file(file_path);
    if (!file.is_open()) {
        cerr << "<ERROR>: Failed to open file! " << file_path << endl;
...
void Complex::write_complex_to_file(vector<set<string>>& complexes,
                                    string& file_path) {
    ofstream file(file_path);
    if (!file.is_open()) {
        cerr << "<ERROR>: Failed to open file! " << file_path << endl;
...
void Complex::write_complex_to_file(set<set<string>>& complexes,
                                    string& file_path) {
    ofstream file(file_path);
    if (!file.is_open()) {
        cerr << "<ERROR>: Failed to open file! " << file_path << endl;
...
double Complex::calculate_cohesion(const UnGraph& g, set<string>& _proteins) {
    ...
    for (auto& e : g.edges) {
        if (!_proteins.count(e->node_a->protein_name) ||
            !_proteins.count(e->node_b->protein_name)) {
            continue;
        }
    ...
    for (auto& p : _proteins) {
        temp_cohesion +=
            sum[p] * (count[p] + 1) / static_cast<int>(_proteins.size());
    }
...
void Complex::update_complexes(vector<Complex>& complexes,
...
double complex_match_score1(const set<string>& s1, const set<string>& s2) {
    ...
    return static_cast<double>(common.size()) /
           static_cast<double>(max(s1.size(), s2.size()));
}
...
void Complex::update_complexes(vector<set<string>>& complexes,
    ...
    for (auto& c : complexes) {
        if (complex_match_score1<string>(c, complex) >= threshold) {
...
bool Complex::evaluate_by_density(UnGraph& g, set<string>& complex, double threshold) {
    int count = 0;
    for (auto it_p = complex.begin(); it_p != complex.end(); ++it_p) {
        auto p_ptr = g.ID2Protein[g.protein_name_id[*it_p]];
        for (auto& nei : p_ptr->neighbor) {
            auto pp_ptr = g.ID2Protein[g.protein_name_id[nei->protein_name]];
            if(p_ptr->neighbor.count(pp_ptr)) {
                count += 1;
            }
        }
    }
    double density = static_cast<double>(count) / static_cast<double>(complex.size() * (complex.size() - 1));
    return density >= threshold;
}

void UnGraph::split_graph1(queue<UnGraph>& ppi_queue,
                           vector<UnGraph>& splited_ppi) {
    UnGraph current_ppi = ppi_queue.front();
    ppi_queue.pop();
    ...
    while (true) {
        ...
        vector<string> new_edge_list;
        vector<double> edge_weight;
        for (auto it = current_ppi.proteins.begin();
             it != current_ppi.proteins.end(); ++it) {
...
void UnGraph::get_complexes1(UnGraph& origin_ppi, UnGraph& g,
                             vector<set<string>>& complexes,
                             double similarity_threshold) {
    if (g.proteins.empty())
        return;
...
map<string, set<string>> UnGraph::read_protein_go(string file_path) {
    fstream file(file_path);
    map<string, set<string>> protein_go;
    string line;
    while(getline(file, line)) {
        istringstream iss(line);
        string protein;
        ...
        protein_go.insert(make_pair(protein, go));
    }
    file.close();
    return protein_go;
}