...
:	file(file),m_nLowerSize(lowersize),m_dLowerDensity(density),m_dLowerIncrease(lowerincrease),
 	graphMode(mode),m_pNeighbor(NULL),m_pWeight(NULL)
{
	// pass through the file for the first time
	size_t i,j, linecnt=0;
	string sa,sb, linevalue;
	float weight;
	...
	while(true)
	{
		fin.getline(tempchar,MAX_LINE_LENGTH,'\n');
		if(fin.eof()) break;

		linevalue = tempchar;
		linecnt++;

		istringstream iss(tempchar);

		// read in interactor A, B
		iss.getline(tempchar, MAX_NAME_LENGTH,'\t');
		sa = tempchar;
		if(iss.fail())
		{
			cerr << "Error: cannot read interactor A for line " << linecnt << ":\t" << linevalue << endl;
			exit(1);
		}
		...
		if(iss.fail())
		{
			cerr << "Error: cannot read interactor B for line " << linecnt << ":\t" << linevalue << endl;
			exit(1);
		}

		// read in float value interaction weight
		iss >> weight;
		if(iss.fail())
		{
			cerr << "Error: cannot read float weight for line " << linecnt << ":\t" << linevalue << endl;
			exit(1);
		}

		if (weight <=0 || weight > 1)
		{
			cerr << "Warning: jump illegal interaction weight " << weight << " for line " << linecnt << endl;
			continue;
		}

		if ( sa == sb )
		{
			cerr << "Warning: jump self interaction:\t"<< sa << '\t' << sb << endl;
			continue;
		}
	...
	// ignore this message
	//if (weightsum < m_dLowerDensity ){cerr << "Warning: low average edge confidence " << weightsum << endl;}
	...
	if (mode == '1'){
		...
		for (i=0;i<m_nEdge;)
		{
			if (mode =='2')
			{
				...
				// don't output warning message for now
