...
###
 # @brief: 
 # @Author: jh
 # @Date: 2024-06-11 07:11:11
 # @LastEditTime: 2024-06-11 07:11:14
### 

# 设置输入数据文件和结果文件的路径
input_file="/home/jh/code/JHPC/dataset/Yeast/PPI/Gavin.txt"
...
for p1 in $(seq $min_threshold $step $max_threshold); do
    for p2 in $(seq $min_threshold $step $max_threshold); do
        for p3 in $(seq $min_threshold $step $max_threshold); do
            result_file="${result_dir}/Gavin_${p1}_${p2}_${p3}.txt"
